...
const ArticleForm = ({title, checkingAuthentication, fetchArticles, type, func, article, getArticleForEditing, tags: tagsData, handleTagsFunc, addTagFunc}) => {
    ...
    const [tagsArr, setTagsArr] = useState(type === 'new' ? ['tag', ''] : article?.tagList.filter(el => el.trim() !== ''));
    ...
    let tags = tagsArr.map((el, i, arr) => {
        ...
        return (
            <div key={i} className={classes.tagInputWrapper}>
                ...
                />
                <div className={classes.tagDelete} onClick={ () => {
                    console.log('delete')
                    console.log(tagsArr);
                    setTagsArr((arr) => {

                        let newTagsArr = [...arr.slice(0, i), ...arr.slice(i+1)]
                        if (newTagsArr.length === arr.length) {newTagsArr.pop()}
                        console.log(newTagsArr);
                        return arr.length === 1 ? arr : newTagsArr;
                    });
                } }>Delete</div>
                {
                    (i === tagsArr.length - 1)  && <div className={classes.tagAdd} onClick={() => {
                        console.log('add');
                       // addTagFunc();
                       setTagsArr((arr) => {
                           return [...arr].concat('')
